...
@section('title','Operator Completed Trip')
@section('content')
			...
			<div class="row">
				...
				<div class="col-xl-3 col-lg-4 col-md-6 col-12 d-flex">
					<div class="box red w-100">
						...
						<h4>{{ date("d-m-Y", strtotime($quote->booking->trip_end_date)) }}</h4>
					</div>
				</div>
						...
						@if($isRate)
							@foreach($rating as $rate)
								<div class="bottom">
									<div class="row">
										<div class="col-12">
										<p class="border-0">{{ $rate->user->name }}</p>
										</div>
										
										<div class="col-lg-6 col-12">
										
											Rate: 
											<div name="rating" id="rating" data-rate-value="0" style="width: 134.531px; height: 30px; position: relative; cursor: default; user-select: none;">
												<span>☆</span>
											</div>
											<p>{{ $rate->comments }}</p>
